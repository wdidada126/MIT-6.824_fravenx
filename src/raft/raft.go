...
func (rf *Raft) applier() {
	...
	defer rf.mu.Unlock()
	for rf.killed() == false {
		if len(rf.installSnapshotMsg.Snapshot) > 0 {
			msg := ApplyMsg{
				CommandValid:  false,
				Snapshot:      rf.installSnapshotMsg.Snapshot,
				SnapshotValid: true,
				SnapshotIndex: rf.installSnapshotMsg.SnapshotIndex,
				SnapshotTerm:  rf.installSnapshotMsg.SnapshotTerm,
			}
